...
const DashboardPage: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleShare = (e: React.MouseEvent, quiz: Quiz) => {
    e.stopPropagation();
    try {
      // Encode the entire quiz object to allow sharing without a backend
      const jsonString = JSON.stringify(quiz);
      const encodedData = btoa(encodeURIComponent(jsonString));

      // Construct a portable URL that includes the data
      const url = `${window.location.origin}/#/quiz/${quiz.id}?data=${encodedData}`;
      ...
      alert("Could not generate share link for this assessment.");
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div className="space-y-2">
            ...
            <p className="text-gray-400 font-medium">
              You have {quizzes.length} active projects in your library.
            </p>
          </div>
          <Button
          ...
          {filteredQuizzes.length === 0 ? (
            <div className="col-span-full py-32 text-center">
              <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-gray-100">
                <HiOutlineClipboardDocumentList size={20} color="gray" />
              </div>
              <h3 className="text-2xl font-black text-gray-900 mb-2">
                No assessments found
              </h3>
              <p className="text-gray-400 font-medium mb-8">
                Start by creating a new quiz from a template or using AI.
              </p>
            ...
            filteredQuizzes.map((quiz) => (
              <Card
                ...
                className="group h-full flex flex-col overflow-hidden relative"
              >
                <div
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 p-2 bg-black/10 text-white rounded-xl opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 backdrop-blur-md"
                  >
                    <FiTrash2 />
                  </button>
                </div>
                ...
                <div className="p-8 flex-grow flex flex-col">
                  ...
                  <div className="flex items-center gap-4 text-xs font-bold text-gray-400 mb-8">
                    ...
                    <span className="flex items-center gap-1.5">
                      <HiOutlineClipboardDocumentList /> {quiz.questions.length}{" "}
                      Qs
                    </span>
                  </div>
                  ...
                  <div className="mt-auto space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        ...
                        className={`!rounded-xl transition-all ${
                          copiedId === quiz.id
                            ? "scale-105 shadow-indigo-200"
                            : ""
                        }`}
                        icon={copiedId === quiz.id ? <FiCheck /> : <FiShare2 />}
                      >
                    ...
                    <Button
                      ...
                      className="w-full !rounded-xl text-gray-400 font-bold"
                      icon={<FiBarChart2 />}
                    >
                      Analytics
                    </Button>
