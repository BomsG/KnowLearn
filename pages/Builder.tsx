...
const BuilderPage: React.FC<{ user: User }> = ({ user }) => {
  ...
  const handleBannerUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Basic size check to keep shared links from becoming excessively long
      if (file.size > 200000) {
        // 200KB limit for sharing via URL
        alert(
          "Image is a bit large for sharing. Try an image under 200KB to ensure shared links work in all browsers."
        );
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        setQuiz({ ...quiz, coverImage: reader.result as string });
      };
  ...
  return (
    <div className="bg-gray-50/50 min-h-screen pb-32">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Card
          className="p-12 mb-12 border-t-[8px] overflow-hidden"
          style={{ borderTopColor: quiz.themeColor }}
        >
          <div className="flex flex-col md:flex-row justify-between items-start gap-12">
                ...
                {THEME_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full transition-all ${
                      quiz.themeColor === c
                        ? "scale-125 border-4 border-white shadow-sm"
                        : "hover:scale-110"
                    }`}
            ...
            <div
              className="w-full md:w-48 h-48 bg-gray-50 rounded-[2rem] border-4 border-dashed border-gray-100 flex flex-col items-center justify-center text-gray-300 group cursor-pointer hover:border-indigo-100 transition-all overflow-hidden relative"
              onClick={() => !quiz.coverImage && fileInputRef.current?.click()}
            >
              {quiz.coverImage ? (
                <>
                  <img
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setQuiz({ ...quiz, coverImage: undefined });
                    }}
                    className="absolute top-2 right-2 p-2 bg-white/90 backdrop-blur shadow-sm rounded-full text-red-500 hover:bg-red-50 transition-all"
                  >
                    <FiX />
                  </button>
                </>
        ...
        <div className="space-y-12">
          {quiz.questions.map((q, idx) => (
            <Card
              key={q.id}
              className="p-12 relative group/q transition-all hover:border-gray-200"
            >
              ...
              <div className="flex gap-6 mb-12">
                <div className="w-12 h-12 rounded-2xl bg-gray-900 text-white flex items-center justify-center font-black shrink-0">
                  {idx + 1}
                </div>
                <input
              ...
              {q.type === "mcq" && (
                <div className="space-y-3 mb-12 md:ml-12">
                  {q.options?.map((opt, oIdx) => (
                    <div
                      key={opt.id}
                      className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl group/opt border border-transparent hover:border-gray-100"
                    >
                      <button
                        onClick={() =>
                          updateQ(q.id, {
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover/opt:opacity-100 p-2 text-gray-200 hover:text-red-500 transition-all"
                      >
                        <FiTrash2 />
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="md:ml-4 text-xs font-black text-indigo-600 flex items-center gap-2 mt-4 hover:translate-x-1 transition-transform"
                  >
                    <FiPlus /> Add Option
                  </button>
                </div>
              ...
              <div className="mt-12 pt-8 border-t border-gray-50 flex justify-between items-center">
                <div className="flex gap-8">
                  <div className="flex items-center gap-2 text-gray-400">
                    ...
                    <input
                      ...
                      value={q.timer || ""}
                      onChange={(e) =>
                        updateQ(q.id, { timer: parseInt(e.target.value) || 0 })
                      }
                    />
                  </div>
                  <div className="flex items-center gap-2 text-gray-400">
                    ...
                    <input
                      ...
                      value={q.points}
                      onChange={(e) =>
                        updateQ(q.id, { points: parseInt(e.target.value) || 0 })
                      }
                    />
                  </div>
                </div>
                <Button
                  ...
                  size="sm"
                  icon={<FiZap />}
                  loading={isReviewing === q.id}
                  onClick={() => handleAiReview(q)}
                >
                  AI Optimization
                </Button>
              </div>
