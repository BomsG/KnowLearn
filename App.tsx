...
const AppContent: React.FC = () => {
  ...
  const isStandaloneRoute = standaloneRoutes.includes(location.pathname) || isPublicQuizRoute;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        <Routes>
          ...
          <Route path="/create" element={user ? <CreateQuizPage /> : <Navigate to="/login" />} />
          <Route path="/builder/:id?" element={user ? <BuilderPage user={user} /> : <Navigate to="/login" />} />
          <Route path="/quiz/:id" element={<QuizPlayerPage />} />
          <Route path="/analytics/:id" element={user ? <AnalyticsPage /> : <Navigate to="/login" />} />
